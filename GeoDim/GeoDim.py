...
def GeoDim(row):
    ...
    Xdim = 0.0029036507*dis*px  # oneplus
    # feature vertical dimension
    #Ydim = 0.0058313294*dis*py
    Ydim = 0.0029036507*dis*py  # oneplus
    # SCALING FACTOR SHOULD BE TUNED AFTER TESTING CAMERA FOV AND RANGEFINDER

    result = [row[7], row[8], row[13], row[14], Flat,
              Flon, Falt, xmin, ymin, xmax, ymax, Xdim, Ydim]
    return result
...
for entry in files:
    with open('GeoDim/GeoDim/' + entry, mode='r') as readable_file:
        next(readable_file)
        GD_reader = csv.reader(readable_file, delimiter=',',
                               quotechar='"', quoting=csv.QUOTE_MINIMAL)
        for row in GD_reader:
            data.append(GeoDim(row))


header = ['image', 'image_path', 'label', 'confidence', 'FeatureLat',
          'FeatureLon', 'FeatureAlt', 'xmin', 'ymin', 'xmax', 'ymax', 'Xdim', 'Ydim']
